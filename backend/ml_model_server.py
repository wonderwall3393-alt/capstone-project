#!/usr/bin/env python3
"""
ML Model Server - Using actual model_telco_recommendation.pkl
Processes survey data through ML model with proper feature encoding
"""
    ...
    def process_survey_through_model(self, survey_data):
        ...
        for i, package in enumerate(PACKAGES):
            if probabilities is not None and i < len(probabilities):
                ...
                ml_score = (phone_score + budget_score) / 12.0  # Normalize to 0-1
...
class MLRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def health_check(self):
        ...
        health_data = {
            'status': 'healthy',
            'ml_model_loaded': self.ml_processor.model_loaded,
    ...
    def handle_ml_recommendation(self):
        """Handle ML-based recommendation request"""
        ...
        try:
            ...
            # Process through ML model with weighting logic
            recommendations = self.ml_processor.process_survey_through_model(survey_data)

            # Count ML recommendations
            ...
            response = json.dumps({
                ...
                'metadata': {
                    'ml_model_used': self.ml_processor.model_loaded,
                    'feature_encoding': 'active',
                    'weighting_logic': 'active',
                    'total_recommendations': len(recommendations),
                    'ml_count': ml_count,
                    'survey_count': 0,
                    'recommendation_source': 'ML Model with Weighting Logic',
...
def run_server():
    """Initialize and run the ML server"""
    # Initialize database
    init_database()

    # Create and run server
    PORT = 8001
    with socketserver.TCPServer(("", PORT), MLRequestHandler) as httpd:
        print(f"Sphinx Net ML Recommendation Server running at http://localhost:{PORT}")
        print("ML Integration: model_telco_recommendation.pkl + Feature Encoding + Weighting Logic")
        print("Available endpoints:")
        print("  GET  /api/packages - Get all available packages")
        print("  GET  /api/health - Check ML model status")
        print("  POST /api/recommend - Get ML model recommendations")
        print("\nFeatures:")
        print("   Actual ML model predictions using model_telco_recommendation.pkl")
        print("   Feature encoding for all survey responses")
        print("   Weighting logic: Budget (35%) + Usage (30%) + Need (20%) + Tech (15%)")
        print("   Phone model-based feature encoding as per tech_doc.txt")
        httpd.serve_forever()
